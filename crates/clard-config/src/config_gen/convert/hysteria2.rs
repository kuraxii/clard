...
pub fn convert(line: &str) -> Result<Value, ConfigGenError> {
    ...
    let parsed =
        parse_url_like(after_scheme).ok_or_else(|| ConfigGenError::InvalidNode("hysteria2 URI 解析失败".into()))?;
    ...
    let name =
        decode_and_trim(parsed.fragment.as_deref()).unwrap_or_else(|| format!("Hysteria2 {}:{port}", parsed.host));
    ...
    kv_opt(&mut m, "sni", q.get("sni").or_else(|| q.get("peer")).map(Value::from));
