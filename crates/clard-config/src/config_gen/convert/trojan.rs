...
use super::helpers::{
    Query, decode_and_trim, kv, kv_opt, parse_bool_or_presence, parse_port_or_default, parse_url_like,
...
pub fn convert(line: &str) -> Result<Value, ConfigGenError> {
    ...
    let parsed =
        parse_url_like(after_scheme).ok_or_else(|| ConfigGenError::InvalidNode("trojan URI 解析失败".into()))?;
    ...
    let name = decode_and_trim(parsed.fragment.as_deref()).unwrap_or_else(|| format!("Trojan {}:{port}", parsed.host));
    ...
    if q.has("skip-cert-verify") {
        kv(
            &mut m,
            "skip-cert-verify",
            parse_bool_or_presence(q.get("skip-cert-verify")),
        );
    }
    kv_opt(
        &mut m,
        "fingerprint",
        q.get("fingerprint").or_else(|| q.get("fp")).map(Value::from),
    );
    ...
    kv_opt(
        &mut m,
        "client-fingerprint",
        q.get("client-fingerprint").map(Value::from),
    );
